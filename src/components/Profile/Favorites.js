...
const Favorites = () => {
  ...
  const checkStockLS = ({ item }) => {
    if (checkStock(item.id) >= item.stock) {
      showMsg("Disculpa, ya tenés la cantidad límite de compra para este producto", false);
    } else {
      addCart({ item });
    }
  };
      ...
      <Box className="d-flex justify-content-between">
        <Box className="bg-light rounded-4 p-3 w-100 m-2">
          ...
          {!userFavorites.length ? (
            <Typography>Agregá acá los productos que te gustaron para verlos más tarde.</Typography>
          ) : (
